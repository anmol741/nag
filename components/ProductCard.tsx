...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border border-ink/10 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 w-full shrink-0 overflow-hidden bg-cream">
        ...
        <CompareButton productId={product.id} variant="icon" className="absolute left-3 top-3 z-10 bg-white/90" />
        <WishlistButton productId={product.id} className="absolute right-3 top-3 z-10 bg-white/90" />
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <Link href={`/products/${product.slug}`} className="mt-3 block">
          <h3 className="font-display text-base leading-snug text-ink group-hover:text-gold-dark">
            {product.name}
          </h3>
        </Link>
